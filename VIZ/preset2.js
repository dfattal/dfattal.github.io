...
async function main() {
  ...
  async function visualizeFile(file) {
    ...
    if (file) {
      ...
      await parseObjectAndCreateTexturesForPointCloud(views, debugTexture);
      console.log('views', views);
      console.log('stereo render data', lifInfo.stereo_render_data);

      // Now that we know if mono or stereo setup webGL
      if (views.length < 2) {
        pointCloudRenderer = new PointCloudRenderer(canvas, views[0]);
      } else {
        pointCloudRenderer = new PointCloudRenderer(canvas, views[0], views[1]);
        //pointCloudRenderer = new PointCloudRenderer(canvas, views[0]); // mono test
      }
  ...
  function updateRenderCamPosition(phase) {
    ...
    // invd = focus * (views[0].layers[0].invZ.min - views[0].layers[0].invZ.max) + views[0].layers[0].invZ.max; // set focus point
  ...
  async function render() {
    if (!saving) {
      ...
      if (views.length < 2) {
        pointCloudRenderer.render(renderCam);
      } else {
        pointCloudRenderer.render(renderCam);
      }
      stats.end();
