...
// info views
uniform sampler2D uImage; // for LDI this is an array
...
float readDisp(sampler2D iChannel, vec2 uv, float vMin, float vMax) { return texture(iChannel, vec2(clamp(uv.x,2.0/iRes.x,1.0-2.0/iRes.x),clamp(uv.y,2.0/iRes.y,1.0-2.0/iRes.y))).x * (vMin - vMax) + vMax; }
...
// Action !
vec4 raycasting(vec2 s2, mat3 FSKR2, vec3 C2, mat3 FSKR1, vec3 C1, sampler2D iChannelCol, sampler2D iChannelDisp, float invZmin, float invZmax, float t) {
    ...
    for (int i = 0; i < numsteps; i++) {
        ...
        disp = readDisp(iChannelDisp, s1 + .5,invZmin,invZmax);
...
void main(void) {
    ...
    if ((abs(uv.x-.5)<.5*newDim.x) && (abs(uv.y-.5)<.5*newDim.y)) {
        ...
        mat3 FSKR1 = matFromFocal(vec2(f1/iRes.x,f1/iRes.y))*matFromSkew(sk1)*matFromRoll(roll1)*matFromSlant(sl1);

        vec3 C2 = uFacePosition;
        mat3 FSKR2 = matFromFocal(vec2(f2/oRes.x,f2/oRes.y))*matFromSkew(sk2)*matFromRoll(roll2)*matFromSlant(sl2);

        vec4 result = raycasting(uv-0.5, FSKR2, C2, FSKR1, C1, uImage, uDisparityMap, invZmin, invZmax, 1.0);
