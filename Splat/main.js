...
async function main() {
    ...
    if (navigator.xr) {
        try {
            ...
            if (isVRSupported) {
                ...
                xrButton.addEventListener('click', onRequestXRSession);
            } else {
                xrButton.textContent = 'VR Not Supported';
            }
        } catch (error) {
            console.error('Error checking for XR support:', error);
            xrButton.textContent = 'VR Error';
        }
    } else {
    ...
    async function onRequestXRSession() {
        try {
            // Request a VR session with viewer reference space
            ...
            // Use the same GL context for both VR and non-VR
            xrGL = gl;

            // Create the WebGL layer for rendering to the headset
            const xrLayer = new XRWebGLLayer(xrSession, xrGL);
            xrSession.updateRenderState({
                baseLayer: xrLayer
            });

            // Get reference space and start rendering loop
            xrReferenceSpace = await xrSession.requestReferenceSpace('local-floor');
            isXRSession = true;

            // Set up XR controller input
            setupXRControllers();

            // Start the XR render loop (which will replace our normal render loop)
            xrSession.requestAnimationFrame(xrFrame);

            // Update button
            xrButton.textContent = 'Exit VR';
            xrButton.removeEventListener('click', onRequestXRSession);
            xrButton.addEventListener('click', onEndXRSession);
    ...
    // Function to set up XR controllers
    function setupXRControllers() {
        if (!xrSession) return;

        // Track controller sources
        const controllerSources = [];

        // Handle adding new input sources
        xrSession.addEventListener('inputsourceschange', (event) => {
            // Add new sources
            for (let source of event.added) {
                controllerSources.push(source);
                console.log('Input source added:', source);
            }

            // Remove old sources
            for (let source of event.removed) {
                const index = controllerSources.indexOf(source);
                if (index >= 0) {
                    controllerSources.splice(index, 1);
                }
    ...
    function xrFrame(time, frame) {
        ...
        if (!pose) {
            // No pose available, skip this frame
            return;
        }

        // Get the WebGL layer for the XR session
        const xrLayer = xrSession.renderState.baseLayer;

        // Bind to the WebGL framebuffer for XR
        gl.bindFramebuffer(gl.FRAMEBUFFER, xrLayer.framebuffer);

        // Process XR controller input
        ...
        for (let source of inputSources) {
            // Skip input sources without gamepad
            if (!source.gamepad) continue;

            const gamepad = source.gamepad;

            // Handle thumbstick input from controllers
            if (gamepad.axes.length >= 4) {
                // Left controller (typical mapping)
                if (source.handedness === 'left') {
                    // Movement with left thumbstick
                    if (Math.abs(gamepad.axes[0]) > 0.1) {
                        moveX += gamepad.axes[0] * 0.05; // Strafe left/right
                    }
                    if (Math.abs(gamepad.axes[1]) > 0.1) {
                        moveZ -= gamepad.axes[1] * 0.05; // Move forward/back
                    }
                }

                // Right controller (typical mapping)
                if (source.handedness === 'right') {
                    // Rotation with right thumbstick
                    if (Math.abs(gamepad.axes[0]) > 0.1) {
                        rotateY += gamepad.axes[0] * 0.05; // Turn left/right
                    }
                }
            }

            // Handle primary/trigger buttons if needed
            if (gamepad.buttons.length > 0) {
                // Implement button actions if needed
            }
        }

        // Apply controller-driven movement to reference space
        if (moveX !== 0 || moveZ !== 0 || rotateY !== 0) {
            // Create a transform from the movement input
            const moveTransform = new XRRigidTransform(
                { x: -moveX, y: 0, z: -moveZ },
                { x: 0, y: Math.sin(rotateY / 2), z: 0, w: Math.cos(rotateY / 2) }
            );

            // Update the reference space with this transform
        ...
        for (const view of pose.views) {
            ...
            // Set the projection matrix from the view's projection matrix
            gl.uniformMatrix4fv(u_projection, false, view.projectionMatrix);

            // Set the view matrix from the view's transform
            ...
            // Extract approximate focal length from projection matrix
            // For a typical perspective projection matrix:
            // Focal length x = projection[0][0] * (width/2)
            // Focal length y = projection[1][1] * (height/2)
            const fovFactor = 1.0; // May need adjustment based on testing
            const focalX = Math.abs(view.projectionMatrix[0] * viewport.width * 0.5 * fovFactor);
            const focalY = Math.abs(view.projectionMatrix[5] * viewport.height * 0.5 * fovFactor);
            gl.uniform2fv(u_focal, new Float32Array([focalX, focalY]));

            // Calculate and send the combined view projection matrix to the worker for sorting
            const viewProj = multiply4(view.projectionMatrix, viewMatrix);
    ...
    // Function to end XR session
    function onEndXRSession() {
        if (xrSession) {
            // Just trigger the session to end - cleanup will happen in onXRSessionEnded
            xrSession.end();
            // Don't modify button text or event listeners here - wait for the 'end' event
        }
    }

    // Function to handle when XR session ends
    function onXRSessionEnded() {
        ...
        // Reset button state
        xrButton.textContent = 'Enter VR';
        xrButton.removeEventListener('click', onEndXRSession);
        xrButton.addEventListener('click', onRequestXRSession);

        // Reset canvas size based on browser window 
        // (may have been modified during XR session)
        resize();

        // Restart the regular rendering loop
